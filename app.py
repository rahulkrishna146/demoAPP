...
client = boto_session.client("bedrock-runtime", region_name="us-east-1")
...
image_upload = st.file_uploader("Upload an image",type=['jpeg','png','jpg','pdf'])
image_uploaded = ''# empty string
# if image input:
if image_upload is not None:
    #student_answer = 'Please consider the image aswell text for student answer'
...
This is a question from the chapter {chapter}. {partial}
...
model_prompt = prompt.invoke({"grade":grade, "subject": subject, "max_marks": max_marks,"chapter": chapter, "partial": partial, "question": question, "accepted_answer": accepted_answer, "grading_criteria": grading_criteria, "student_answer": student_answer, "image_uploaded": image_uploaded})
